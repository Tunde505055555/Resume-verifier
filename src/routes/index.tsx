...
function SubmissionCard({
  ...
  const finalized = submission.finalized ?? Boolean(submission.verdict);
    ...
    <Card>
      <CardHeader className="pb-3">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          {finalized ? (
            <Badge
              variant={submission.verified ? "default" : "destructive"}
              className="gap-1.5"
            >
              <BadgeCheck className="h-3.5 w-3.5" />
              {submission.verified ? "Claims supported" : "Claims disputed"}
            </Badge>
          ) : (
        ...
        <div className="mt-2 flex flex-wrap gap-2 text-[11px] text-muted-foreground">
          <span className="rounded-md border border-border px-2 py-0.5">
            {finalized ? "Finalized · verdict locked forever" : "Open · anyone may trigger verification"}
          </span>
          {submission.submitter && (
            <span className="rounded-md border border-border px-2 py-0.5">
              submitted by {short(submission.submitter)}
            </span>
          )}
          {finalized && submission.verifier && (
            <span className="rounded-md border border-border px-2 py-0.5">
              finalized by {short(submission.verifier)}
            </span>
          )}
        </div>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        ...
        {finalized ? (
          <p className="text-xs text-muted-foreground">
            This verdict is final. Re-verification is rejected on-chain, so no further
            action is possible.
          </p>
        ) : (
          <Button
