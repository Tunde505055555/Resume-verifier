...
const wait = (client: any, hash: unknown) =>
  client.waitForTransactionReceipt({ hash, status: "FINALIZED", interval: 5000, retries: 240 });
...
async function main() {
  ...
  // Finality of the transaction and application of state can lag; poll the
  // on-chain view until the submission reports itself finalized.
  let isFinal = false;
  for (let i = 0; i < 60 && !isFinal; i++) {
    isFinal = (await read(submitterClient, "is_finalized", [id])) === true;
    if (!isFinal) await new Promise((r) => setTimeout(r, 5000));
  }
  ok("is_finalized view reports FINALIZED state", isFinal);
