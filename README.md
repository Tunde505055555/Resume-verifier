...
- **Deployed contract:** `0x0CF5eB453F865ab5691D1E8D2ED2Eaf18f2B7bF2`
...
```sh
bun scripts/e2e-finality.ts                                              # deploy + test
bun scripts/e2e-finality.ts 0x0CF5eB453F865ab5691D1E8D2ED2Eaf18f2B7bF2   # test the live one
```

The test deploys the contract, reads the policy, submits a resume from wallet A, finalizes
it from wallet B (proving permissionlessness), waits for `FINALIZED` (not merely
`ACCEPTED`) and polls the `is_finalized` view, asserts the stored verdict is structurally
...
The full, unedited output of the run that produced the deployment the frontend points at
is committed as [`scripts/e2e-finality.log`](scripts/e2e-finality.log) — contract
`0x0CF5eB453F865ab5691D1E8D2ED2Eaf18f2B7bF2`, submission `0`, exit code `0`, every check
PASS, including:

```text
PASS  is_finalized view reports FINALIZED state
PASS  permissionless caller could finalize
PASS  verifier wallet attributed  "0x4330254E2508Bdfa74aBeeC9DC418AdEA9FCc7e5"
PASS  submitter recorded separately
